...
EOF
	...
	mcopy -i ${WORKDIR}/${BOOT_IMG} -s ${TMPDIR}/../../../../kernel/debian_kernel/arch/arm/boot/zImage ::
	mcopy -i ${WORKDIR}/${BOOT_IMG} -s ${TMPDIR}/../../../../kernel/debian_kernel/arch/arm/boot/dts/rk3288-miniarm.dtb ::
	mmd -i ${WORKDIR}/${BOOT_IMG} ::/extlinux
	mcopy -i ${WORKDIR}/${BOOT_IMG} -s ${TMPDIR}/../../../../kernel/rk3288.conf ::/extlinux/extlinux.conf
	mcopy -i ${WORKDIR}/${BOOT_IMG} -s ${TMPDIR}/../../../../kernel/hw_intf.conf ::
...
generate_loader1_image () {
	...
	dd if=${TMPDIR}/../../../../uboot/debian_u-boot/u-boot.img of=${GPTIMG} conv=notrunc,fsync seek=64
}
