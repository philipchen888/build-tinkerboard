...
void rk_mmc_set_clk( int freq )
{
    ...
    *CRU_CLKSEL_CON11 = (MMC0_PLL_MASK | MMC0_DIV_MASK) << 16 | (mux << MMC0_PLL_SHIFT | (src_clk_div - 1) << MMC0_DIV_SHIFT);
}
...
int dwmci_data_transfer( struct mmc_data *data, int fifo_mode )
{
    int ret;
    int i;
    int size;
    ...
    for (;;) {
        ...
        if ( fifo_mode && size ) {
            len = 0;
            if ( data->flags == MMC_DATA_READ && (mask & DWMCI_INTMSK_DTO) ) {
                while ( size ) {
                    len = *DWMCI_STATUS;
...
int dwmci_send_cmd( int hasdata, struct mmc_data *data )
{
    ...
    *DWMCI_RINTSTS = DWMCI_INTMSK_ALL;
    ...
    if ( hasdata ) {
        ...
        if ( fifo_mode ) {
            ...
            *DWMCI_RINTSTS = DWMCI_INTMSK_TXDR | DWMCI_INTMSK_RXDR;
            *DWMCI_INTMASK |= DWMCI_INTMSK_TXDR | DWMCI_INTMSK_RXDR;
            ...
            *DWMCI_BYTCNT = len;
            ...
            blk_cnt = data->blocks;
            dwmci_wait_reset( DWMCI_CTRL_FIFO_RESET );
            *DWMCI_DBADDR = (unsigned int)&cur_idmac;
            ...
            while( 1 ) {
                ...
                cur_idmac.next_addr = (unsigned int)&cur_idmac + sizeof(struct dwmci_idmac);
                if (blk_cnt <= 8)
                    break;
                blk_cnt -= 8;
                i++;
    ...
    *DWMCI_CMD = flags;
    ...
    if ( hasdata ) {
        ret = dwmci_data_transfer( data, fifo_mode );
...
int mmc_send_if_cond( void )
{
    ...
    cmd.cmdarg = ((cfgvoltages & 0xff8000) != 0) << 8 | 0xaa; 
