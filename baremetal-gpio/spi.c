...
void spi_write( unsigned char *dout, int len )
{
    ...
    *SPI2_ENR = 0x1;
    ...
    towrite = len;
    while (towrite) {
	if (towrite && !(*SPI2_SR & SR_TF_FULL)) {
            *SPI2_TXDR_BASE = out ? *out++ : 0;
	    towrite--;
	}
    }
    while (*SPI2_SR & SR_BUSY) {
        udelay( 1 );
    }
...
void rk_spi_init( int speed )
{
        ...
        //freq = 19800000; //myrk_spi_calc_modclk( 5000000 );
        src_clk_div = 29; //594000000 / freq - 1;

        *SPI2_BAUDR = 0x2;
        *CRU_CLKSEL_CON39 = ((SPI2_PLL_MASK | SPI2_DIV_MASK) << 16) | (SPI2_PLL_SELECT_GENERAL << SPI2_PLL_SHIFT | src_clk_div << SPI2_DIV_SHIFT);
...
void ssd1306_test ( void )
{
    ...
    *GPIO5B = (0xc0 << 16);                       // GPIO5B3 alt = 0, GPIO
    *GPIO5_SWPORTA_DDR |= 0x800;                  // GPIO5B3 OUTPUT
    *GPIO5_SWPORTA_DR |= 0x800;                   // DC = 1
    *GPIO8A = (0xf0c0 << 16) | 0x5040;            // SPI2, GPIO8A3,6,7
    *GPIO8B = (0xf << 16) | 0x5;                  // SPI2, GPIO8B0,1
